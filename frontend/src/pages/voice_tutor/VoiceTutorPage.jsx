/* ============================================================
   ...
   Real-time voice AI tutor with VAD + interruption support
   ============================================================ */
    ...
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 'var(--space-4)', marginBottom: 'var(--space-6)' }}>
        {[
          ...
          { label: 'INTERACTIONS',     value: stats.questionsAsked * 2 },
        ].map((s) => (
...
export default function VoiceTutorPage() {
  ...
  const { isPlaying: audioPlaying, playBase64, interrupt } = useAudioPlayer({
    ...
    onPlayEnd: () => {
      ...
      if (useVoiceTutorStore.getState().isSessionActive) {
  ...
  const { isRecording, audioLevel, startRecording, stopRecording, hasPermission } = useVoiceRecorder({
    ...
    onRecordingComplete: async ({ base64, format }) => {
      ...
      incrementQuestions()
      send({ type: 'audio', audio: base64, format: format.ext, session_id: sessionId })
    },
  })
  ...
  const { connect, disconnect, send, status: wsStatus } = useWebSocket({
    ...
    onOpen: () => {
      ...
      toast.success('[ WS ] Connected to AI tutor')
    },
    ...
    onMessage: async (data) => {
      if (data.type === 'connected') {
        addTranscript({
          role: 'tutor',
          text: 'Hello! I am your AI tutor. I use the Socratic method — guiding you with questions and hints. What would you like to learn today?',
        })
  ...
  }, [vadState, isSessionActive, isRecording, audioPlaying])
  ...
  const handleInterrupt = useCallback(() => {
    if (audioPlaying) {
      ...
      stopRecording()
      setVadState(VAD_STATE.LISTENING)
      toast('[ INTERRUPT ] Tutor interrupted', { icon: '🎤' })
    }
  }, [audioPlaying, interrupt, stopRecording, setVadState])

  /* ── START SESSION ────────────────────────────────────── */
  const handleStartSession = async () => {
    toast.loading('[ INIT ] Starting session...', { id: 'session-toast' })

    let sessionId = null

    try {
      const result = await voiceTutorService.createSession({ subject, grade_level: gradeLevel })
      // Handle both response formats
      sessionId = result?.data?.session_id || result?.session_id
    } catch (err) {
      // Demo mode
    }
    ...
    // Build WebSocket URL with confirmed session ID
    const wsUrl = `${import.meta.env.VITE_WS_BASE_URL || 'ws://localhost:8000'}/api/v1/voice-tutor/ws/${sessionId}`

    // Add system message before connecting
    addTranscript({
      role: 'system',
    ...
    try {
      connect(wsUrl)
      toast.success('[ READY ] Session started!', { id: 'session-toast' })
    } catch (err) {
      // Fallback demo mode without WebSocket
      setWsConnected(false)
      setVadState(VAD_STATE.LISTENING)
      addTranscript({
        role: 'tutor',
        text: `Hello! I am your AI tutor for ${subject}. I use the Socratic method — guiding you with questions and hints rather than giving direct answers. What topic would you like to explore today?`,
      })
      toast.success('[ DEMO ] Demo session started', { id: 'session-toast' })
    }
  }
  ...
  const sendDemoMessage = (text) => {
    ...
    if (wsStatus === WS_STATUS.OPEN) {
      ...
      setTimeout(() => {
        const responses = [
          `Great question! Before I answer, can you tell me what you already know about "${text.slice(0, 30)}..."?`,
          `Interesting! Let me guide you. What do you think causes this? Try to reason it out first.`,
          `You're thinking in the right direction! Now, what principle would you apply here?`,
          `Good question! Let me give you a hint: think about what we discussed earlier. How does that connect?`,
        ]
        const resp = responses[Math.floor(Math.random() * responses.length)]
        addTranscript({ role: 'tutor', text: resp })
      }, 1000)
    }
  }
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 'var(--space-12) var(--space-8)', gap: 'var(--space-8)' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-data)', color: 'var(--muted)', maxWidth: '500px', lineHeight: 1.7 }}>
                Speak naturally. Your AI tutor uses the Socratic method — guiding you with questions and hints. VAD auto-detects your voice. Interrupt the tutor anytime.
              </p>
            </div>
              ...
              <div style={{ padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
                ...
                {hasPermission === false && (
                  <div style={{ padding: 'var(--space-3)', background: 'var(--term-red-dim)', border: '1px solid var(--term-red)', borderLeft: '3px solid var(--term-red)', fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-nano)', color: 'var(--term-red)', textTransform: 'uppercase', letterSpacing: 'var(--ls-wide)' }}>
                    ✕ MICROPHONE PERMISSION DENIED — TEXT INPUT WILL WORK
                  </div>
                )}
        ...
        <div style={{ borderLeft: 'var(--border)', display: 'flex', flexDirection: 'column', background: 'var(--surface)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: 'var(--space-6)', borderBottom: 'var(--border)', background: 'var(--term-bg)' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-nano)', textTransform: 'uppercase', letterSpacing: 'var(--ls-wider)', color: vadState === VAD_STATE.SPEAKING ? 'var(--accent-cyber)' : 'var(--term-green)', marginBottom: 'var(--space-4)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span>{vadState === VAD_STATE.SPEAKING ? 'TUTOR OUTPUT' : 'MIC INPUT'}</span>
              <VADStatus state={vadState} />
            </div>
            <Waveform
            ...
            <div style={{ marginTop: 'var(--space-3)', fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-nano)', textTransform: 'uppercase', letterSpacing: 'var(--ls-wide)', color: wsStatus === WS_STATUS.OPEN ? 'var(--term-green)' : 'rgba(255,255,255,0.3)' }}>
              ● WS: {wsStatus === WS_STATUS.OPEN ? 'CONNECTED' : wsStatus === WS_STATUS.CONNECTING ? 'CONNECTING...' : 'DEMO MODE'}
            </div>
          </div>
